...
Architecture authority for the emulator core:

- `docs/adr-headless-machine-kernel.md` defines the required
  MachineKernel architecture for V1/V2/V3. It supersedes local
  fastloader patching as the core strategy: one kernel owns time,
  VICE-compatible cross-domain bus semantics, canonical tracing, and
  no hidden TrueDrive rescue hooks.
