...
| `GCR_conv_data[16]` encode table | gcr.c:51-57 | **— MISSING** | **MISSING — Phase 2 port** |
...
|---|---|---|---|
| `gcr_convert_4bytes_to_GCR` | 68-86 | **— MISSING** | **MISSING — Phase 2 port** (4→5 byte encoder, used by sector_to_GCR + write_sector) |
...
rows** (target 30+; will expand in Phase 2 sub-row matrices).

Open Phase 2: 4 MISSING ports + 6 re-audit rows + write-back coupling.