...
- **Open (BUG-049):** the audio stutter is a regression in this delivery/hosting
  path (post-2026-05-24 "perfect"; Spec 744 daemon-rearchitecture window), NOT in
  the emulation or the audio files. Reliable next probe: a de-confounded full-loop
  measurement (in-memory telemetry, async flush) of fps / event-loop delay / ring
  fill / per-client send cost — or a git bisect by ear from `f889479c`.
