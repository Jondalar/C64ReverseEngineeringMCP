...
export class ProjectKnowledgeService {
  ...
  renderDocs(scope: "all" | "findings" | "entities" | "open-questions" | "anti-patterns" | "project-profile" = "all"): { written: string[] } {
    ...
    // Spec 847 D4 — a generated document carries what it was rendered FROM, so its
    // staleness is computable instead of silent. Ultima VI's FINDINGS.md rendered 19 of
    // that project's 41 findings, three days old, and said nothing about either number.
    const counts = {
      findings: this.listFindings().length,
      entities: this.listEntities().length,
      questions: this.listOpenQuestions().length,
    };
    const header = "<!-- generated by c64re render_docs; do not edit by hand -->\n\n";
    const stamp = (title: string): string =>
      generatedFrontmatter(title, counts) + "\n" + header;
    if (scope === "all" || scope === "findings") {
      ...
      writeFileSync(path, stamp("Findings") + lines.join("\n") + "\n");
      written.push(path);
    }
    if (scope === "all" || scope === "entities") {
      ...
      writeFileSync(path, stamp("Entities") + lines.join("\n") + "\n");
      written.push(path);
    }
    if (scope === "all" || scope === "open-questions") {
      ...
      writeFileSync(path, stamp("Open Questions") + lines.join("\n") + "\n");
      written.push(path);
    }
    if (scope === "all" || scope === "anti-patterns") {
      ...
      writeFileSync(path, stamp("Anti-Patterns") + lines.join("\n") + "\n");
      written.push(path);
    }
