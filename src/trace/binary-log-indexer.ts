...
import { streamCaptureEvents, readCaptureHeader } from "./capture-stream.js";
...
// Spec 746.x / BUG-052 — STREAMING decode lives in `capture-stream.ts` and is
// shared with the loader-lens readers: the .c64retrace is never read whole and the
// decoded stream is never materialized. This file only folds rows into DuckDB.
...
export async function indexBinaryLog(
  ...
  // BUG-035 — the header carries the format version; v1 mem-access records decode
  // one byte shorter than v2, and `streamCaptureEvents` applies it for us.
  const { meta, size } = readCaptureHeader(retracePath);
  ...
  try {
    ...
    // One streaming pass over the log (capture-stream.ts owns the window loop).
    streamCaptureEvents(retracePath, onEvent);
    ...
    // store (the .c64retrace remains the authority and is re-indexable). The log's
    // own handle is owned and released by `streamCaptureEvents`.
...
export function readBinaryLogMeta(retracePath: string): TraceFileMeta {
  return readCaptureHeader(retracePath).meta;
}
