...
Accordingly this server REFUSES runtime discovery shortcuts: runtime_loader_lens, runtime_trace_start and the trace-analysis tools require a read-derived hypothesis (a concrete $address + what you read that points there), and the payload-extraction doors refuse a standard-GCR medium (it is a static depack). Read the code first; trace, if at all, only to confirm. The graph tools (graph_find, graph_node, graph_edges, graph_path, graph_overview) ARE the static analysis, indexed: query them before reading a full listing — "the listing is too long" is where the flight to runtime starts.`;
