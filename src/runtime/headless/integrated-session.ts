...
export class IntegratedSession {
   ...
   */
  captureVicPresentation(shallow = false, omitFramebuffer = false): RuntimeCheckpointVicPresentation {
    // Spec 765 — shallow hands the LIVE framebuffer refs to the checkpoint ring
    // (zero-copy; the ring detaches). omitFramebuffer drops the two ~162 KiB
    // framebuffers entirely (null) — the always-on perma-anchor uses this, since
    // the framebuffer is a derivable shadow regenerated by re-running one frame
    // on restore (746 §1). The small continuation scalars are always kept.
    const fb = omitFramebuffer ? null : this.literalPortFb;
    const fbStable = omitFramebuffer ? null : this.literalPortFbStable;
    return {
      literalPortFb: fb ? (shallow ? fb : fb.slice()) : null,
      literalPortFbStable: fbStable ? (shallow ? fbStable : fbStable.slice()) : null,
