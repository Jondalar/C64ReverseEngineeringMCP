...
export async function runMonitorCommand(ctx: MonitorShellCtx, command: string): Promise<MonitorResult> {
  ...
  try {
    ...
    // chis [cycles] | chis <start> <end> — replay from the nearest ring
    // checkpoint with capture ON, render the window as a swimlane (history is
    // REGENERATED by replay, not stored). Non-destructive. One-arg form =
    // last N cycles up to now (default 5000); two-arg form (BUG-046) = an
    // arbitrary historical cycle window between two checkpoints.
    if (op === "chis") {
      if (!ctx.traceRead) return { error: "chis: trace-read bridge unavailable (run via the daemon)" };
      const n1 = tokens[1] !== undefined ? parseInt(tokens[1], 10) : NaN;
      const n2 = tokens[2] !== undefined ? parseInt(tokens[2], 10) : NaN;
      const margs: Record<string, unknown> = Number.isFinite(n1) && Number.isFinite(n2)
        ? { cycleStart: n1, cycleEnd: n2 }
        : { windowCycles: Math.max(1, Number.isFinite(n1) ? n1 : 5000) };
      try { return { output: await ctx.traceRead("chis", margs) }; }
