...
export function registerHeadlessTools(server: McpServer, context: ServerToolContext): void {
  ...
  server.tool(
    ...
    safeHandler("runtime_session_start", async ({
      ...
      // Spec 836 D1 — an ATTACH never changes the shared machine's medium.
      //
      // BUG-041 made media_path work for a cartridge and this call was left
      // unconditional, so a second session passing its own .crt did not get its
      // own machine — it swapped the medium under the human who was using
      // theirs. There is a dedicated door for a deliberate swap
      // (runtime_media_mount, or the monitor's swapcrt), so the accidental one
      // has no reason to exist.
      let refusedLine: string | undefined;
      if (r.attached && absMedia) {
        refusedLine =
          `NOT mounted: ${absMedia} — this call ATTACHED to the shared machine, which is already running ` +
          `${r.diskPath || "(no medium)"}, and the human is co-driving it. ` +
          `To change the shared machine's medium on purpose: runtime_media_mount (or swapcrt in runtime_monitor). ` +
          `To run YOUR medium on a machine of your own: runtime_sandbox_run, or runtime_scene_reel for a scripted run — each a private daemon on its own port, ending on a budget.`;
      }
      let openedLine: string | undefined;
      if (absMedia && !r.attached) {
        try {
  ...
  server.tool(
    "runtime_load_prg",
    "Inject a PRG into a session's RAM as if KERNAL LOAD placed it. Use to load a program without a disk. Not for disk LOAD (mount + runtime_type a LOAD line) or static analysis (use analyze). Inputs: session_id, prg_path, optional load_address. Returns: load range.",
    {
      session_id: z.string(),
      prg_path: z.string(),
