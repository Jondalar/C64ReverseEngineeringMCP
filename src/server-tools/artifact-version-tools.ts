...
export function registerArtifactVersionTools(server: McpServer, ctx: ServerToolContext): void {
  ...
  server.tool(
    ...
    "List every known version of a single source subject (one payload / listing identity) with its role, format, rank, and status, and which one is current. Use when you need to see the alternatives for a payload's .asm/.tas source — for example before opening or before choosing a different version. The subject id is where the sources live plus the stem they share (e.g. \"analysis/disk/wl/02_2.0\" for 02_2.0_disasm.asm + 02_2.0_semantic.tas in that directory); pass an artifact id and the tool resolves its subject, or pass a bare filename and it lists the subjects carrying it when more than one does. Not for listing all artifacts in the project (use list_artifacts) and not for reading file bytes (use read_artifact). Returns the version group: current artifact, source (auto/manual), needs-decision flag, and the ordered version list.",
    {
  ...
  server.tool(
    ...
    "Get the current best version artifact for a single source subject — the one the UI opens by default. Use when you want the right .asm/.tas to read or act on for a payload, without choosing among versions yourself: it returns the manually-pinned current if one exists, otherwise the highest-ranked source (curated/semantic beats generated). Pass the subject id (the directory plus the shared stem, e.g. 'analysis/disk/wl/02_2.0') or any artifact id belonging to the subject; a bare filename that several disks carry is answered with the list of subjects rather than one of them picked for you. Not for listing the alternatives (use list_artifact_versions) and not for changing the current (use set_current_artifact_version). Returns the current artifact's id, path, role, and format.",
    {
      project_dir: z.string().optional().describe("Project root directory. Absolute or project-relative; defaults to the active project."),
      subject_id: z.string().describe("The subject id — the directory the sources live in plus their shared stem (e.g. 'analysis/disk/wl/02_2.0') — OR an artifact id belonging to the subject. A bare filename resolves when only one subject carries it."),
    },
  ...
  server.tool(
    ...
    "Pin one version as the current best source for a subject. Use when the auto-picked current is wrong — for example a hand-made source should win over a generated dump, or a specific version is the canonical one. The choice persists in the project knowledge store as a manual decision and a later project_inventory_sync will respect it (it will not be auto-overwritten). Pass the subject id (the directory plus the shared stem, e.g. 'analysis/disk/wl/02_2.0') or any artifact id of the subject, plus the artifact id to make current — the artifact decides which group it is pinned in. Not for marking a version obsolete (use mark_artifact_version_stale) and not for reading versions (use list_artifact_versions). Returns the updated version group.",
    {
      project_dir: z.string().optional().describe("Project root directory. Absolute or project-relative; defaults to the active project."),
      subject_id: z.string().describe("The subject id — the directory the sources live in plus their shared stem (e.g. 'analysis/disk/wl/02_2.0') — OR an artifact id belonging to the subject. A bare filename resolves when only one subject carries it."),
...
// Accept a subject id, an artifact id, or a bare filename stem; resolve to the
// subject.
//
// A bare stem is no longer an identity: `pl0` names one subject on a one-sided
// project and three on a three-sided one, and the version tools used to answer
// with whichever listing sorted first — another disk's code, presented as this
// one's current version. So a stem that names several subjects is a refusal
// that lists them, and the caller picks.
function resolveSubjectOrAsk(service: ProjectKnowledgeService, input: string):
  { subjectId: string } | { ambiguous: string[] } {
  const resolved = service.resolveSubjectRef(input);
  if (resolved.candidates.length > 1) return { ambiguous: resolved.candidates };
  return { subjectId: resolved.subjectId };
}

function ambiguousText(input: string, candidates: string[]): string {
  return [
    `"${input}" is a filename, not a subject — ${candidates.length} subjects in this project carry it:`,
    ...candidates.map((c) => `  ${c}`),
    `Pass one of those as subject_id (they are the directory the source lives in plus its stem), or pass the artifact id.`,
  ].join("\n");
}
