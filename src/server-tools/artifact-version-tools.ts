...
export function registerArtifactVersionTools(server: McpServer, ctx: ServerToolContext): void {
  ...
  server.tool(
    ...
    "List every known version of a single source subject (one payload / listing identity) with its role, format, rank, and status, and which one is current. Use when you need to see the alternatives for a payload's .asm/.tas source — for example before opening or before choosing a different version. The subject id is the base stem shared by a payload's sources (e.g. \"02_2.0\" for 02_2.0_disasm.asm + 02_2.0_semantic.tas); pass an artifact id and the tool resolves its subject. Not for listing all artifacts in the project (use list_artifacts) and not for reading file bytes (use read_artifact). Returns the version group: current artifact, source (auto/manual), needs-decision flag, and the ordered version list.",
    {
