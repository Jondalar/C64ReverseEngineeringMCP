...
export async function verdict(projectDir: string): Promise<Verdict> {
  ...
  //
  // Spec 877 moved the computation into `contract/promises.ts`, because a door that has to
  // REFUSE on a promise needs the promise as a record — an id a waiver can name, the number
  // asked, the number measured, the way to close the gap — and a blocker string carries
  // none of that. The wording below is the same wording it always was: it is what the 849
