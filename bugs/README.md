...
| BUG-031 | Disk-layout view ignores registered-payload `medium_spans` — shows only CBM manifest + BAM, so code-derived raw disk regions (no dir entry) never appear on the disk geometry (Wasteland disk looks empty save prodos/2.0). Disk-view analogue of BUG-024. | knowledge / ui-v3 | medium | fixed (disk-layout builder overlays sector-kind payload `medium_spans` as origin=custom files + geometry, dedup vs CBM cells — `e2e:bug031` 8/8) |
