...
| BUG-029 | Trace-store reads (`runtime_swimlane_slice`/`trace_store_*`) fail "Could not set lock" while the daemon process is live (DuckDB cross-process lock); store reads fine once the daemon exits. Fix = route trace reads through the daemon (746.5). | trace-store / runtime daemon | medium | open |
