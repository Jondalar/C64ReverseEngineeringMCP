...
| BUG-023 | Custom true-drive (drive-side GCR) writes not persisted to host .d64/.g64 file (Wasteland Utils Copy + Scramble HighScore) | runtime | high | fixed (root cause: no RAM-media→backing-file writeback; added `persistMountedDiskToFile` + `runtime_media_persist` + unmount writeback; `smoke:023-host-file` 8/8 re-reads host file + mtime) |
