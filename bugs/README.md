...
| BUG-028 | Runtime input/drive tools not daemon-routed: `runtime_type` (et al.) fail "No integrated session" on the shared daemon session while `runtime_session_status`/`runtime_render_screen` work — can read shared session, can't drive it | mcp-tool / runtime | high | open |
