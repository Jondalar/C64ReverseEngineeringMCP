...
| BUG-034 | Effective-segments overlay (Spec 055) not reused: annotation segment reclassifications (e.g. a mis-disassembled data region → `data`) show ONLY in `disasm_prg`, not in `inspect_address_range` / memory-map / graphics / annotated-listing / disk-layout views; `resolve-pc` re-implements the overlay incorrectly (append-not-overlay). Render-private algorithm duplicated 3×. | analysis / knowledge views | medium | open (root-caused; fix = Spec 751 option D — one shared server-side `effective-segments` module routed through all 6 consumers + "reclassified by annotation" hint from Spec 055 findings; non-destructive, no `_analysis.json` write-back) |
