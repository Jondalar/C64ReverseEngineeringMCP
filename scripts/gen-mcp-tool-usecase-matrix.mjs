...
const PATH_TOOLS = new Set([
  ...
  // retired-name-ok: the matrix documents the aliases so the old name resolves to the new one
  "disasm", "analyze", "analyze_prg", "disasm_prg", "disasm_raw", "disasm_menu", "assemble_source",
  "write_annotations", "merge_annotations",
...
const CURATED = {
  ...
  start_re_workflow: { role: "workflow", swimlane: "entry-project-baseline", useWhen: "Select the workflow/profile (crack, port, analysis, bugfix) for the project objective.", notFor: "Per-PRG static disassembly — use run_prg_reverse_workflow / disasm.", adjacent: ["agent_set_role", "agent_onboard"], e2e: ["E2E-A"] },
  ...
  project_inventory_sync: { role: "workflow", swimlane: "entry-project-baseline", useWhen: "Sync the project store with disk: register present-but-untracked files (media, payloads, sidecars, generated + hand-written source), import manifests, and rebuild every view. Run after extraction/disassembly/import or when onboarding/audit reports unregistered files or stale views.", notFor: "Creating a project — use project_init; extracting bytes from media — use extract_disk / extract_crt.", adjacent: ["project_init", "build_all_views", "agent_onboard"], e2e: ["E2E-A", "E2E-B"] },
  unregister_files: { role: "workflow", swimlane: "entry-project-baseline", useWhen: "Take artifact rows back out of the store after a bulk of machine output was registered by mistake — those rows add their bytes to the coverage denominator and nothing to what is understood, and declaring the glob `intentional` afterwards only stops NEW registrations.", notFor: "Deleting a file — this never touches disk; hiding infrastructure from the UI — that is the internal flag; removing a row somebody has written about — it refuses those and says why.", adjacent: ["project_inventory_sync", "register_existing_files", "scan_registration_delta"], e2e: ["E2E-A"] },
  agent_next_step: { role: "workflow", swimlane: "entry-project-baseline", useWhen: "Ask the server for the single next product step (+ branch alternatives) derived from real project state — the orchestrated 'what do I do now?'.", notFor: "Running the step — use agent_run_step or the named product tool; recording progress — use agent_record_step.", adjacent: ["agent_run_step", "project_inventory_sync", "agent_onboard"], e2e: ["E2E-A", "E2E-B"] },
  agent_run_step: { role: "workflow", swimlane: "entry-project-baseline", useWhen: "Run a workflow step from agent_next_step: executes the inventory/media-sync step in-process and points at the product tool for every other step.", notFor: "Choosing the step — use agent_next_step; calling internal maintenance helpers — they are wrapped behind product facades.", adjacent: ["agent_next_step", "project_inventory_sync"], e2e: ["E2E-A", "E2E-B"] },
  ...
  list_payloads: { role: "knowledge-read", swimlane: "entry-project-baseline", useWhen: "List executable payloads discovered in media.", notFor: "Disassembling one payload — use disasm.", adjacent: ["list_artifacts", "disasm"], e2e: ["E2E-A"] },
  ...
  propose_annotations: { role: "disassembly", swimlane: "disassembly-improve", useWhen: "Write a draft annotation set (labels/segments/routine docs) for review.", notFor: "Applying + rebuilding — use disasm with annotations.", adjacent: ["disasm", "save_finding"], e2e: ["E2E-C"] },
  ...
  merge_annotations: { role: "disassembly", swimlane: "disassembly-improve", useWhen: "Fold several readings of one payload into one annotations file — every contradiction refused by naming both sides, every resolution recorded in the project as a finding saying who claimed what, which one won and why.", notFor: "One reading — use write_annotations; a draft to review by hand — use propose_annotations.", adjacent: ["write_annotations", "disasm", "read_finding"], e2e: ["E2E-C"] },
  ...
  register_payload: { role: "knowledge-write", swimlane: "asset-linking", useWhen: "Register a carved code-derived/custom-loader load (no CBM dir / no on-disk LUT) as a first-class payload — pass source_prg_path, load_address, format and medium_spans (track/sector) so it renders on the disk view at its T/S, in the memory map at its load address, and in list_payloads with load/fmt/src/asm.", notFor: "Thin name-only records — never use save_entity kind=payload; ASM-only links — use link_payload_to_asm.", adjacent: ["link_payload_to_asm", "list_payloads", "disasm"], e2e: ["E2E-D"] },
  ...
  // The useWhen here said "Map BAM/sector allocation" until issue #24. It never read the
  // BAM — ownership comes from the extraction manifest — and the wrong half of that
  // sentence is the half that cost someone a disk: on a track/sector-addressed medium the
  ...
  disasm_menu: { role: "disassembly", swimlane: "disassembly-improve", useWhen: "Disassemble a specific menu/region with chosen entry points.", notFor: "Whole-PRG disasm — use disasm.", adjacent: ["disasm"], e2e: ["E2E-C"] },
  inspect_address_range: { role: "static-analysis", swimlane: "disassembly-improve", useWhen: "Inspect a byte/address range of a payload statically.", notFor: "Live memory — use runtime_monitor_memory.", adjacent: ["analyze", "runtime_monitor_memory"], e2e: ["E2E-C"] },
  // Spec 823 — the knowledge graph's five doors (thin over src/knowledge-graph).
  graph_find: { role: "knowledge-read", swimlane: "disassembly-improve", useWhen: "Resolve a name, address, register or ROM entry to graph nodes with stable ids — the start of any structural question.", notFor: "Where a topic is described in prose — use project_search.", adjacent: ["graph_node", "graph_edges", "project_search"], e2e: ["E2E-C"] },
  graph_node: { role: "knowledge-read", swimlane: "disassembly-improve", useWhen: "The card for one node: label vs human name, edge counts, hardware/ROM/ZP touched, runtime observation.", notFor: "Walking the neighbourhood — use graph_edges.", adjacent: ["graph_find", "graph_edges"], e2e: ["E2E-C"] },
  graph_edges: { role: "knowledge-read", swimlane: "disassembly-improve", useWhen: "Callers, callees, readers, writers, references, ROM/ZP/hardware use, indirect accesses — direction × kind × origin.", notFor: "A route between two nodes — use graph_path.", adjacent: ["graph_node", "graph_path", "inspect_address_range"], e2e: ["E2E-C"] },
  graph_path: { role: "knowledge-read", swimlane: "disassembly-improve", useWhen: "Shortest control-flow path between two nodes with per-hop evidence.", notFor: "A node's direct edges — use graph_edges.", adjacent: ["graph_edges", "graph_find"], e2e: ["E2E-C"] },
  graph_overview: { role: "knowledge-read", swimlane: "entry-project-baseline", useWhen: "The structural map of a project: entries, IRQ handlers, banking sites, hot hardware/ROM/ZP, unresolved indirect accesses.", notFor: "One node — use graph_node.", adjacent: ["graph_find", "project_status"], e2e: ["E2E-C"] },
  assemble_source: { role: "disassembly", swimlane: "validation", useWhen: "Assemble ASM (KickAss/64tass) to verify a byte-identical rebuild.", notFor: "Disassembly — use disasm.", adjacent: ["disasm"], e2e: ["E2E-C"] },
  // Spec 829 — a BASIC PRG is not 6502; disasm renders its token bytes as garbage.
  basic_list: { role: "static-analysis", swimlane: "disassembly-improve", useWhen: "Read a BASIC V2 PRG: walk its line records, detokenise, name the PETSCII control codes, and report the SYS targets that hand control to machine code.", notFor: "A machine-code payload — use analyze then disasm.", adjacent: ["disasm", "analyze", "basic_tokenize"], e2e: ["E2E-C"] },
  basic_tokenize: { role: "static-analysis", swimlane: "disassembly-improve", useWhen: "Turn BASIC V2 source text back into a .prg — a loader stub, or an edited listing. The round trip with basic_list is byte-identical.", notFor: "Assembling 6502 source — use assemble_source.", adjacent: ["basic_list", "assemble_source"], e2e: ["E2E-C"] },
  c64ref_lookup: { role: "static-analysis", swimlane: "disassembly-improve", useWhen: "Look up a KERNAL/BASIC/hardware address or symbol meaning.", notFor: "Resolving a runtime PC to source — use runtime_resolve_pc.", adjacent: ["runtime_resolve_pc", "disasm"], e2e: ["E2E-C"] },
  ...
  build_annotated_listing_view: { role: "view-docs", swimlane: "disassembly-improve", useWhen: "Render the annotated disassembly listing view.", notFor: "Raw disasm text — use disasm.", adjacent: ["disasm", "build_memory_map"], e2e: ["E2E-C"] },
  ...
  run_prg_reverse_workflow: { role: "workflow", swimlane: "disassembly-improve", useWhen: "Run the end-to-end per-PRG analyze→disasm→verify workflow.", notFor: "Project-level workflow selection — use start_re_workflow.", adjacent: ["analyze", "disasm"], e2e: ["E2E-C"] },
  ...
  // Spec 716.3 — the door that gets you a machine at all. Named here rather than left
  // to the namespace default, because "part of the headless runtime facade" is exactly
  // wrong for it: it is what you reach for when there is no runtime to have a facade over.
  runtime_install: { role: "runtime-control", swimlane: "entry-project-baseline", useWhen: "No runtime daemon is installed and a runtime tool just said so. Fetches the pinned TRX64 build for this platform, checksum-verified, into a per-version cache.", notFor: "A daemon you already have (set C64RE_TRX64_BIN) or one already running (set C64RE_RUNTIME_ENDPOINT); and it does not supply ROMs, which are yours.", adjacent: ["runtime_session_start", "runtime_session_status"], e2e: ["E2E-B"] },
  runtime_monitor_disasm: { role: "runtime-monitor", swimlane: "freeze-inspect", useWhen: "Disassemble around the live PC at a paused state.", notFor: "Static PRG disasm — use disasm.", adjacent: ["runtime_monitor_registers", "runtime_resolve_pc"], e2e: ["E2E-D"] },
  ...
  runtime_resolve_pc: { role: "runtime-monitor", swimlane: "disassembly-improve", useWhen: "Resolve a runtime PC to a payload/source location for annotation evidence.", notFor: "Symbol/ROM lookup — use c64ref_lookup.", adjacent: ["disasm", "c64ref_lookup"], e2e: ["E2E-C"] },
  ...
  // These two had curated text in the OUTPUT and no entry here, so the committed
  // matrix was hand-edited and the next regeneration silently demoted both to the
  // per-role template ("Part of the Headless runtime facade."). Restored verbatim from
  // the committed rows: the generator is the source, the docs are the artifact.
  runtime_vic_inspect_region: { role: "runtime-inspect", swimlane: "freeze-inspect", useWhen: "Ask what an on-screen OBJECT is made of: a rectangle of the frozen picture resolved to the distinct VIC/RAM sources drawing it.", notFor: "One pixel — use runtime_vic_inspect_at. Where the bytes came FROM — use runtime_vic_origin.", adjacent: ["runtime_vic_inspect_at", "runtime_vic_origin", "runtime_render_screen"], e2e: ["E2E-D"] },
  runtime_vic_origin: { role: "runtime-inspect", swimlane: "freeze-inspect", useWhen: "Tie something visible on screen back to where its bytes came from — an exact byte-hash match against the mounted medium, plus what the project already knows about that asset.", notFor: "What a pixel IS right now — use runtime_vic_inspect_at. A whole object's composition — use runtime_vic_inspect_region.", adjacent: ["runtime_vic_inspect_at", "runtime_vic_inspect_region", "save_finding"], e2e: ["E2E-D"] },
  ...
  // Spec 730.1/730.2 — promoted Disk/G64 raw-inspection + cartridge-chunk RE
  // tools. Curated so they land in product swimlanes (not the format-forensics /
  // maintenance catch-alls) and carry e2e tags for the disk-raw / cart-chunk
  // product gates.
  list_g64_slots: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "List the half-track slots present in a .g64 image before deeper raw inspection.", notFor: "Decoded DOS directory — use inspect_disk.", adjacent: ["inspect_g64_track", "inspect_disk"], e2e: ["E2E-E"] },
  inspect_g64_track: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Inspect one raw G64 (half-)track's GCR/sync structure.", notFor: "DOS file listing — use inspect_disk.", adjacent: ["inspect_g64_blocks", "scan_g64_headers"], e2e: ["E2E-E"] },
  inspect_g64_blocks: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Decode the GCR block layout of a raw G64 track.", notFor: "Sync gaps only — use inspect_g64_syncs.", adjacent: ["inspect_g64_syncs", "inspect_g64_track"], e2e: ["E2E-E"] },
  inspect_g64_syncs: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Inspect sync marks / gaps in a raw G64 track.", notFor: "Block decode — use inspect_g64_blocks.", adjacent: ["inspect_g64_blocks"], e2e: ["E2E-E"] },
  scan_g64_headers: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Scan a G64 track for sector headers (incl. non-standard ones).", notFor: "Reading a candidate sector's bytes — use read_g64_sector_candidate.", adjacent: ["read_g64_sector_candidate", "inspect_g64_track"], e2e: ["E2E-E"] },
  read_g64_sector_candidate: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Read the decoded bytes of one candidate sector found by a header scan.", notFor: "Bulk raw extraction — use extract_g64_sectors.", adjacent: ["scan_g64_headers", "extract_g64_sectors"], e2e: ["E2E-E"] },
  extract_g64_sectors: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Extract decoded raw sectors from a G64 track into project artifacts.", notFor: "A whole raw track image — use extract_g64_raw_track.", adjacent: ["extract_g64_raw_track", "read_g64_sector_candidate"], e2e: ["E2E-E"] },
  extract_g64_raw_track: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Extract a whole raw GCR track image for custom-loader / protection analysis.", notFor: "Per-sector decode — use extract_g64_sectors.", adjacent: ["extract_g64_sectors", "analyze_g64_anomalies"], e2e: ["E2E-E"] },
  analyze_g64_anomalies: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Detect raw-track anomalies (weak bits, long tracks, killer tracks) signalling copy protection.", notFor: "Normal directory read — use inspect_disk.", adjacent: ["inspect_g64_track", "set_payload_disk_hint"], e2e: ["E2E-E"] },
  suggest_disk_lut_sector: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Suggest the sector LUT a custom loader likely uses, to drive a custom-LUT extraction.", notFor: "Standard DOS extraction — use extract_disk.", adjacent: ["extract_disk_custom_lut"], e2e: ["E2E-E"] },
  extract_disk_custom_lut: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Extract a payload using a custom sector LUT (non-DOS loader layout).", notFor: "Standard DOS files — use extract_disk.", adjacent: ["suggest_disk_lut_sector"], e2e: ["E2E-E"] },
  set_payload_disk_hint: { role: "format-forensics", swimlane: "entry-project-baseline", useWhen: "Tag a payload with a disk-structure hint (drive-code/protected/raw/bad-crc/gap) so the disk heatmap colours it.", notFor: "Sector extraction — use extract_disk / extract_disk_custom_lut.", adjacent: ["analyze_g64_anomalies", "set_payload_disk_hint"], e2e: ["E2E-E"] },
  bulk_create_cart_chunk_payloads: { role: "media-ingress", swimlane: "entry-project-baseline", useWhen: "After extract_crt, promote useful cartridge bank/slot chunks into payload entities for analysis.", notFor: "Single-chunk linking — use link_cart_chunk_to_asm.", adjacent: ["extract_crt", "link_cart_chunk_to_asm"], e2e: ["E2E-F"] },
  link_cart_chunk_to_asm: { role: "knowledge-write", swimlane: "asset-linking", useWhen: "Link a cartridge chunk payload to its best ASM/TASS artifact.", notFor: "Promoting chunks to payloads — use bulk_create_cart_chunk_payloads.", adjacent: ["bulk_create_cart_chunk_payloads", "record_cart_chunk_packer"], e2e: ["E2E-F"] },
  record_cart_chunk_packer: { role: "knowledge-write", swimlane: "asset-linking", useWhen: "Record packer/format metadata for a cartridge chunk.", notFor: "Linking ASM — use link_cart_chunk_to_asm.", adjacent: ["link_cart_chunk_to_asm"], e2e: ["E2E-F"] },
};
...
- Disassemble a payload → \`disasm\` (after \`analyze\`).
