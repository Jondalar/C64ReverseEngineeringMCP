...
session.renderToPng("/tmp/scr-00-ready.png");
console.log("  /tmp/scr-00-ready.png BASIC ready");

console.log("Mount Scramble...");
await mountMedia(session, 8, resolve("samples/scramble_infinity.d64"));

console.log("LOAD\"*\",8,1 + RUN");
session.typeText('LOAD"*",8,1\r');
session.runFor(60_000_000, { cycleBudget: 60_000_000 });
session.renderToPng("/tmp/scr-01-loaded.png");
console.log("  /tmp/scr-01-loaded.png after LOAD (~60s)");

session.typeText("RUN\r");
// Option (a) PNG-stability cut: Scramble title plateaus t090=t120; drop t180.
for (const sec of [10, 30, 60, 90, 120]) {
  session.runFor(sec * 1_000_000, { cycleBudget: sec * 1_000_000 });
  const path = `/tmp/scr-t${sec.toString().padStart(3,"0")}s.png`;
