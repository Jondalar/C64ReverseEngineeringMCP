...
ok(!(await checkTraceDiscipline(undefined)).allowed, "no hypothesis → refused");
ok(!(await checkTraceDiscipline("")).allowed, "empty hypothesis → refused");
ok(!(await checkTraceDiscipline("let me trace everything and see where my input shows up")).allowed, "prose fishing, no address → refused");
ok(!(await checkTraceDiscipline("$C000")).allowed, "bare address, no rationale → refused");
ok(!(await checkTraceDiscipline("find where the manual word is compared")).allowed, "the classic fishing phrasing → refused");

// ALLOWED cases (read-derived hypothesis) ---
ok((await checkTraceDiscipline("$C000 should hold the manual-check result; the input routine at $B800 stores the typed word there (read block2 disasm)")).allowed, "address + real rationale → allowed");
ok((await checkTraceDiscipline("validate the extractor manifest against the read-set at $DD00 — confirm the loader reads T33 sectors")).allowed, "Build-phase validation with address + purpose → allowed");
ok((await checkTraceDiscipline("confirm $1998 SECTOR_LOAD is the only reader of the LUT; I read the $0800 jump table and it dispatches there")).allowed, "Accolade-style read-derived hypothesis → allowed");

// The refusal message is a redirect, not just a wall ---
const r = (await checkTraceDiscipline(undefined)).refusal ?? "";
...
const G = async (h) => await checkRuntimeDiscipline(h, { tool: "runtime_loader_lens", act: "reading a loader-lens landing map" });

// REFUSED cases (fishing) — identical predicate, so the same shapes are blocked ---
ok(!(await G(undefined)).allowed, "loader_lens: no hypothesis → refused");
ok(!(await G("")).allowed, "loader_lens: empty hypothesis → refused");
ok(!(await G("just show me where the packed payload lands")).allowed, "loader_lens: prose fishing, no address → refused");
ok(!(await G("$08D6")).allowed, "loader_lens: bare address, no rationale → refused (the Cybernoid form)");

// ALLOWED cases (read-derived hypothesis) ---
ok((await G("$08D6 is the packed payload; the $0801 BASIC stub JSRs the depacker that writes it (read the boot disasm) — confirm the source block")).allowed, "loader_lens: address + real rationale → allowed");

// The refusal is tool-named and redirects to reading ---
const rr = (await G(undefined)).refusal ?? "";
...
const r2 = (await checkRuntimeDiscipline(undefined, { tool: "trace_store_top_pcs", act: "ranking the hottest PCs (statistics)" })).refusal ?? "";
