...
| 806 | [Retire the TS emulator and VICE](806-retire-ts-emulator-and-vice.md) | READY | Decided 2026-08-12: `ts-emulator` and `runtime/vice` disappear from the released C64RE. The oracle has nothing left to check — the monitor audit put 97 Rust verbs against 87 TS, the drive is proven by `drive_sector_read` plus seven booting titles, and TRX64 carries its own gates. Keeping it costs: 189 files needed a DEPRECATED banner because a reader could not tell product from oracle with the file open. Branch `spec-806-structural-cut`. **Step 1 (evacuate the 33 keepers) and step 2 (convert the tools — `headless.ts` 24 branches, `runtime.ts` 11 + nine whole tools) DONE**; no `isDaemonMode()` fork is left on the tool surface. Open: **step 3, the eleven tools with no counterpart** (five on `drive-session-manager` = one question, three scenario `export/*`, `diagnose_mm`, `session_snapshot`, and `swimlane-render` to evacuate) — all ADVANCED-only, so no default surface is at stake. Then 4 = delete the 173, 5 = retire Spec 723 with its subject, 6 = e2e through the runtime only. | 2026-08-12 |
