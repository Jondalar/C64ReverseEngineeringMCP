...
| 806 | [Retire the TS emulator and VICE](806-retire-ts-emulator-and-vice.md) | DONE | Shipped 2026-08-12 on `spec-806-structural-cut`. **The TypeScript emulator is gone** — 176 files under `src/ts-emulator/`, the 63-file chip-fidelity `tests/` tree that had been importing a dead path since f80b4674, 383 of 544 scripts and 148 of 242 npm entries. **VICE went with it**: `src/runtime/vice/` plus the 49 `vice_*` MCP tools that were still registered on every server start. The eleven tools with no counterpart were **retired, not routed** — the emulator was their only implementation and the runtime has no equivalent object (a C64-less drive session, a structured-JSON snapshot, an `export/*` family); all ADVANCED-only, so the default surface is unchanged at 144. Spec 723 retired into `DOCTRINE.md` with its gate GREEN 25/25 on its last run; `CLAUDE.md` rule 1 is now *one runtime, and it is a separate process*. Surface 302 → 253 tools. Open: **step 6 only** — a fresh-project e2e (boot, monitor, trace, screenshot) through the daemon. | 2026-08-12 |
