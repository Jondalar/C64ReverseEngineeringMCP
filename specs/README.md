...
| 791 | `../TRX64/docs/791-vsf-to-c64re-converter.md` | **VSF → `.c64re` converter** (one-way import onramp) — parse VICE VSF modules → our `Machine` state → emit `.c64re`; runtime untouched (`trx64cli convert-vsf`). C64CART(EF)-first + coarse-VIC cut + **fidelity classification** (`faithful`/`partial`/`inspection-only`, retires the `errors=[]` footgun). NO export (lossy downgrade + harder VIC-write; we're leaving VICE). Our own VICE-faithful port makes it tedious field-mapping, not reconstruction. PROPOSED, building. |
