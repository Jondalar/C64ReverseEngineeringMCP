...
| 611.6 | Port: rotation                              | `rotation.ts` per `docs/vice-1541-arch.md` §13 F (16.16 fixed point, per-cycle hook into drivecpu).                                     | Rotation positions head + emits BYTE-READY edges matching VICE trace at the same drive cycles, against a known-good D64 mounted under `--drive1541=vice`. **Still no game LOAD gate — gcr.ts + drive-image-d64.ts not in place.**     |
| 611.7 | Port: GCR + image formats                   | `gcr.ts` + `drive-image-{d64,g64}.ts` per `docs/vice-1541-arch.md` §13 F + §13 G. P64 = throwing stub.                                  | **First real C64-side gate.** `--drive1541=vice` passes Spec 423 Tier-2 oracle bundle (`smoke-423-{bare-boot,load-directory,motm-canary,krill-loader}.mjs`) + the 5 GREEN-expected games in Spec 601. Pawn + LNR stay RED-expected.   |
