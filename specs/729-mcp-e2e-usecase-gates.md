...
**Status:** DONE (2026-05-30) — all of `npm run check:mcp-product-surface` is GREEN:
tool-boundaries, path-portability, project-inventory (incl. the project_init fix),
the trace writer/reader schema contract (E2E-I, Spec 726 §6a), AND the full live
trace product flow (E2E-B/C/D, `e2e-mcp-trace-first` 22/22). Only deferred gate:
E2E-E change/validation, BLOCKED on Spec 711 (code-overlay tooling) — documented,
not faked.  
...
- **E2E-B/C/D (`scripts/e2e-mcp-trace-first.mjs`) — 22/22.** The full live trace
  PRODUCT FLOW through the default MCP façade over stdio (no `C64RE_FULL_TOOLS`, no
  VICE, no WebSocket, no raw SQL): create a project in an arbitrary dir OUTSIDE the
  repo → copy the seed disk into it → `runtime_session_start(disk_path, trace_out,
  trace_domains)` → `runtime_session_run` toward BASIC → `runtime_mark('basic-ready')`
  → `runtime_type('LOAD"*",8,1\rRUN\r')` → `runtime_session_run` → `runtime_mark
  ('loaded-or-title')` → `runtime_trace_finalize` → query via `trace_store_info` /
  `trace_store_top_pcs` / `runtime_query_events(cpu_step)` → `save_finding` +
  `save_entity` → `build_project_dashboard`. The `trace.duckdb` lands in the
  project's `traces/` dir. Seed disk = `samples/synthetic/1byte.d64` (generated by
  `gen-synthetic-disks.mjs`, copied into the external project — proves media can come
  from any directory, not repo `samples/`). Wired into `check:mcp-product-surface`.

All of `check:mcp-product-surface` is GREEN. The only deliberately deferred gate:

- E2E-E (change/validation) stays expected-PENDING until Spec 711 code-overlay
  tooling lands (documented, not faked).

Known minor artifact: DuckDB leaves a `run.duckdb.wal` beside the store after
finalize (not checkpointed); the store is fully queryable. Tracked as a small
follow-up, not a flow blocker.
